...
const VIRTUAL_SCROLL_CONFIG = {
  ITEM_HEIGHT: 150, // 估计的消息高度
  BUFFER_SIZE: 3,   // 上下缓冲区大小
};

/**
 * VirtuallyScrolledChatArea - 虚拟滚动优化的聊天区域
 * 使用React.memo、useCallback、useMemo等优化性能
 */
const ChatArea: React.FC<ChatAreaProps> = memo(({
  ...
  const visibleRange = useMemo(() => {
    const startIndex = Math.max(0, Math.floor(scrollTop / VIRTUAL_SCROLL_CONFIG.ITEM_HEIGHT) - VIRTUAL_SCROLL_CONFIG.BUFFER_SIZE);
    const endIndex = Math.min(
      messages.length - 1,
      Math.ceil((scrollTop + containerHeight) / VIRTUAL_SCROLL_CONFIG.ITEM_HEIGHT) + VIRTUAL_SCROLL_CONFIG.BUFFER_SIZE
    );
  ...
  const handleTopicSubmit = useCallback((e: React.FormEvent) => {
    e.preventDefault();
    if (!input.trim()) return;
    onRequestChangeTopic(input);
    setInput('');
  }, [input, onRequestChangeTopic]);
  ...
  }, [editTitleInput, sessionTitle, onUpdateSessionTitle]);
  ...
  }, [handleTitleBlur, sessionTitle, topic]);
  ...
  return (
    <div className="flex flex-col h-full bg-white md:rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      >
        <div style={{ height: `${messages.length * VIRTUAL_SCROLL_CONFIG.ITEM_HEIGHT}px`, position: 'relative' }}>
          {visibleMessages.map((msg, index) => (
            <div
              ...
              style={{
                position: 'absolute',
                top: `${(visibleRange.startIndex + index) * VIRTUAL_SCROLL_CONFIG.ITEM_HEIGHT}px`,
                left: 0,
                right: 0,
              }}
            >
          ...
          {isLoading && (
            <div className="flex justify-start" style={{ position: 'absolute', top: `${messages.length * VIRTUAL_SCROLL_CONFIG.ITEM_HEIGHT}px` }}>
