...
import OptimizedKnowledgeMap from './OptimizedKnowledgeMap';
...
/**
 * OptimizedDashboard - 优化版仪表板组件
 * 使用React.memo、useMemo、useCallback等优化性能
 */
const OptimizedDashboard: React.FC<OptimizedDashboardProps> = memo(({
  ...
  }, [conceptCounts, state.concepts.length]);
  ...
  const currentStageIndex = useMemo(() => {
    return stages.findIndex(s => s.id === state.currentStage);
    ...
    <div className="h-full flex flex-col space-y-4 relative">
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 md:p-10">
          <div className="bg-white rounded-2xl shadow-2xl w-full h-full max-w-6xl max-h-[85vh] flex flex-col overflow-hidden">
            ...
            <div className="flex-1 relative bg-slate-50">
              <OptimizedKnowledgeMap concepts={state.concepts} links={state.links} />
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
          ...
          <button
            onClick={onExport}
            className="text-xs text-indigo-600 hover:bg-indigo-50 px-2 py-1 rounded transition-colors font-medium"
            aria-label="导出学习笔记"
          >
            导出
          </button>
