...
// KIMI (月之暗面) 特点：超长上下文支持(200K+)、中文能力强、适合长文档分析
// ============================================
export const KIMI_SYSTEM_INSTRUCTION = `
You are CogniGuide v1.0.6, a Dynamic Personalized Teaching Engine & Knowledge Architect powered by KIMI.
...
**LANGUAGE:** SIMPLIFIED CHINESE (简体中文). KIMI excels at Chinese language understanding.

**KIMI ADVANTAGES:**
- Industry-leading 200K+ context window for long conversations
- Excellent Chinese language comprehension and generation
- Strong memory and context retention across sessions
...
**2. Select Strategy (based on 'teachingMode'):**
   ...
   - **Socratic:** Ask questions. Guide. Don't tell. Use KIMI's natural dialogue abilities.
   - **Narrative:** Storytelling, history, context. Leverage KIMI's storytelling strengths.
   - **Lecture:** Clear, direct definitions and steps. Use KIMI's clear explanation capabilities.

**3. Teaching Stages:**
   - Introduction -> Construction -> Consolidation -> Transfer -> Reflection.

**KIMI OPTIMIZATION:**
- Utilize KIMI's strong Chinese language model capabilities for natural dialogue
- Leverage the ultra-long context window for maintaining conversation continuity
- Use KIMI's understanding of Chinese cultural context when relevant
...
export const sendMessageToKIMI = async (
  history: ChatMessage[],
...
`;
  ...
  const fullMessages = [
    {
      ...
      content: KIMI_SYSTEM_INSTRUCTION
    },
    ...messages.slice(0, -1),
    {
      role: 'user' as const,
  ...
  try {
    ...
    if (!content) {
      throw new Error('No response content from KIMI API');
    }

    // Extract and clean JSON with improved logic
    ...
    if (!validStages.includes(parsed.detectedStage as TeachingStage)) {
      // Infer stage from the last user message
      const lastUserMessage = messages[messages.length - 1]?.content || '';
      if (lastUserMessage.includes('什么是') || lastUserMessage.includes('介绍一下') || lastUserMessage.includes('什么是')) {
        parsed.detectedStage = TeachingStage.Introduction;
      } else if (lastUserMessage.includes('为什么') || lastUserMessage.includes('怎么') || lastUserMessage.includes('如何')) {
